...
def get_chrome_for_testing_url() -> tuple[str, str]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to get Chrome for Testing download URL: {e}") from e
...
def get_chromium_executable_path(install_dir: Path, platform_id: str) -> Path:
    ...
    if system == "Windows":
        ...
        executable = chrome_dir / "Google Chrome for Testing.app" / \
    ...
    if not executable.exists():
        raise FileNotFoundError(
            f"Chrome executable not found at: {executable}")
    ...
    try:
        ...
        if system == "Windows":
            ...
            for chrome_dir in install_dir.glob("chrome-mac*"):
                exe_path = chrome_dir / "Google Chrome for Testing.app" / \
                    "Contents" / "MacOS" / "Google Chrome for Testing"
                if exe_path.exists():
                    return exe_path
...
def install_chromium(install_dir: Optional[Path] = None, clean_existing: bool = True, force_reinstall: bool = False) -> Path:
    ...
    if not force_reinstall:
        ...
        if existing_chrome:
            logger.info(
                "Using existing Chromium installation at: %s", existing_chrome)
    ...
    try:
        ...
        logger.info(
            "Chrome for Testing installed successfully at: %s", executable_path)
